...
public static class Options
{
	...
	public const string BUILD_REPORT_TOOL_DEFAULT_PATH = "Assets/Framework/Plugins/BuildReport";
	...
	static string DefaultOptionsPath
	{
		get
		{
			return string.Format("{0}/../UnityBuildReports/{1}", Application.dataPath, SAVED_OPTIONS_FILENAME);
		}
	}
	...
	static bool IsBuildReportInRegularPaths
	{
		get
		{
			return Directory.Exists(string.Format("{0}/../UnityBuildReports", Application.dataPath)) ||
	...
	static void InitializeOptionsIfNeeded()
	{
		...
		if (string.IsNullOrEmpty(_foundPathForSavedOptions))
		{
			...
			{
				string customBuildReportFolder = BuildReportTool.Util.FindAssetFolder(Application.dataPath + "/Framework/Plugins/", BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME);
